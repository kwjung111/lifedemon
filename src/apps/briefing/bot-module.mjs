...
const moreHousing = /주택.*(?:더|나머지)|(?:더|나머지).*주택/;
const moreJobs = /(?:채용|직업|잡).*?(?:더|나머지)|(?:더|나머지).*?(?:채용|직업|잡)/;
