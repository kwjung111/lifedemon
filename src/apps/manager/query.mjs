...
export function looksLikeManagerQuestion(text) {
  ...
  if (/^\/(?:daemon|system|ask)(?:@\w+)?(?:\s|$)/i.test(value) || /^\/job(?:@\w+)?\s+/i.test(value)) return true;
  if (/(?:해야|할\s*일|챙겨|기억해|메모|저장해|해지|제출|신청해야)/.test(value)) return false;
  return managerTerms.test(value)
    && /(?:[?？]\s*$|왜|어떻게|언제|뭐|무엇|보여\s*줘|확인해|상태|정상|실패했|오류났|문제야)/.test(value);
}
