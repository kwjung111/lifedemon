...
Reply target resolution is deterministic and conversational: explicit numbers may appear anywhere, Korean ordinals and normalized company/source names are accepted, distinctive title terms may identify an item, and a single-item message supplies its own context. Multiple plausible items never trigger a guessed mutation; the domain module asks one short clarification. Common positive, negative, applied, and durable wording stays on this local fast path without an AI dependency.
