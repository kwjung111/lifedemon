...
export function createReminder({ title, dueAt, url = null, module = "global", entityKey = null }) {
  const timestamp = now();
  db.prepare(`
    INSERT INTO reminders(title, due_at, url, module, entity_key, status, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, 'proposed', ?, ?)
    ON CONFLICT(module, entity_key, due_at) DO UPDATE SET
      title=excluded.title, url=COALESCE(excluded.url, reminders.url), updated_at=excluded.updated_at
  `).run(title, dueAt, url, module, entityKey, timestamp, timestamp);
  return db.prepare(`
