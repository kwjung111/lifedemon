...
| 자유문장 의도·대상 분기 | 전역 AI가 목록, Inbox, 알림, 추천 피드백, 추적과 운영 질문을 구분하고 추천 피드백은 제한된 에이전트로 위임 | `test/message-interpreter.test.mjs`, `test/bot-runtime.test.mjs` |
| 복수 추천 피드백 | 에이전트가 필요한 상세를 조회한 뒤 한 문장의 여러 주택·채용 피드백을 독립 도구로 처리 | `test/recommendation-agent.test.mjs` |
...
- 답장 문맥을 전역 라우터와 추천 피드백 에이전트에 포함하므로 도메인별 수정 명령을 외울 필요가 없습니다.
- 고정 명령은 즉시 처리하고 일반 자유문장은 제한된 전역 AI 라우팅 한 번을 사용합니다. 여러 관찰이 필요한 추천 피드백만 명시적인 에이전트 도구 루프를 사용하며 의미 정규식 목록은 늘리지 않습니다.
...
- 일반 자유문장은 제한된 Codex 라우팅 호출 한 번이 필요합니다. 추천 피드백은 목표에 따라 추가 에이전트 판단 호출이 필요하며, 고정 명령과 버튼 콜백은 AI를 사용하지 않습니다.
