...
- `src/apps/navigation/`: Bounded AI intent routing for free-form recommendation-list navigation.
...
The briefing app is a read-only cross-domain presentation layer. Weekday housing and job services prepare all source data before 09:00 without sending standalone digests. `morning-briefing.service` then reads the current domain projections and approved reminders, emits one bounded message, and stores a mixed-domain reply context. Only the top three recommendations per domain are displayed; signatures collapse an unchanged domain to `변경 없음`, while the full analyzed set remains in its owning database. A reply target is resolved across the mixed list and delegated to the owning housing or job module, so application and feedback mutations remain domain-owned. Free-form list and continuation requests are classified by the navigation app through a strict Structured Outputs schema (`show_recommendations`, `next_page`, or `not_navigation`) with a required domain and confidence threshold. It sees only the user's bounded text and public reply-domain metadata, never browses, and cannot mutate application or feedback state. Fixed slash commands remain deterministic.
